...
def cc634_set_section(section: str | None) -> None:
    try:
        if section:
            st.query_params["cc_section"] = section
        elif "cc_section" in st.query_params:
            del st.query_params["cc_section"]
    except Exception:
        pass
    st.session_state["left_section"] = section

def cc634_restore_section() -> None:
    try:
        section = cc634_get_qp("cc_section", "")
        if section and not st.session_state.get("left_section"):
            st.session_state["left_section"] = section
    except Exception:
        pass
...
def inject_clean_theme_css():
    """Single consolidated Candidate Connect web CSS layer.

    Phase 1.2 CSS cleanup rule: keep one global CSS injection here,
    instead of stacking late overrides throughout the app. Local HTML
    tables may still include their own component-level CSS.
    """
    st.markdown("""
<style id="cc-global-css-phase-1-2">
/* === Base Candidate Connect theme/header/sidebar/main controls === */
...
/* === Program selector + multiselect chip readability === */
...
/* === Sidebar visibility safety valve === */
...
/* === Final button/login/password/readability rules === */
...
/* === Tooltip/help popover readability === */
...
def _program_user_labels(campaign_id: str) -> tuple[list[dict], list[str], dict[str, str], dict[str, str]]:
    """Return campaign-scoped people who can receive program/work assignments.

    C4.6.40: Assignment recipients must include BOTH Campaign Organization
    roster people and real app/security users for the same campaign. This lets
    a candidate, campaign admin, manager, or staff user receive a mobile walk
    package even if they were not separately entered as a volunteer.
    """
    ...
    def _email_key(person: dict) -> str:
        return _safe_clean(
            ...
            or (person or {}).get("username")
            or (person or {}).get("login_username")
        ).lower()

    def _person_id_from_seed(seed: str, prefix: str = "auth") -> str:
        seed = _safe_clean(seed).lower()
        return f"{prefix}-" + hashlib.md5(seed.encode("utf-8")).hexdigest()[:10] if seed else ""

    def _campaign_aliases(store: dict) -> set[str]:
        aliases = {campaign_slug}
        try:
            campaigns = (store or {}).get("campaigns") or {}
            current_campaign = campaigns.get(campaign_slug) or {}
            if isinstance(current_campaign, dict):
                for k in ("campaign_id", "id", "name", "campaign", "campaign_name"):
                    v = _safe_clean(current_campaign.get(k))
                    if v:
                        aliases.add(_ops_slug(v))
                        aliases.add(v.lower())
            for cid, c in campaigns.items():
                if not isinstance(c, dict):
                    continue
                vals = [_safe_clean(cid), _safe_clean(c.get("campaign_id")), _safe_clean(c.get("id")), _safe_clean(c.get("name")), _safe_clean(c.get("campaign")), _safe_clean(c.get("campaign_name"))]
                if any(v and _ops_slug(v) == campaign_slug for v in vals):
                    for v in vals:
                        if v:
                            aliases.add(_ops_slug(v))
                            aliases.add(v.lower())
    ...
    def _user_matches_campaign(user_rec: dict, aliases: set[str]) -> bool:
        if not isinstance(user_rec, dict):
            return False
        vals = []
        for key in ("campaign_id", "campaign", "campaign_name", "assigned_campaign", "campaign_slug", "scope_campaign"):
            vals.append(_safe_clean(user_rec.get(key)))
        # Sometimes user records have campaign access in lists/dicts.
        for key in ("campaign_ids", "campaigns", "allowed_campaigns", "campaign_access"):
            raw = user_rec.get(key)
            if isinstance(raw, list):
                for item in raw:
                    vals.append(_safe_clean(item.get("campaign_id") if isinstance(item, dict) else item))
            elif isinstance(raw, dict):
                vals.extend([_safe_clean(x) for x in list(raw.keys()) + list(raw.values())])
        for v in vals:
            if v and (_ops_slug(v) == campaign_slug or v.lower() in aliases or _ops_slug(v) in aliases):
                return True
        return False

    def _security_user_to_person(username: str, user_rec: dict) -> dict:
        username = _safe_clean(username)
        email = _safe_clean(user_rec.get("email") or username)
        name = _safe_clean(
            user_rec.get("name")
            or user_rec.get("full_name")
            or user_rec.get("display_name")
            or user_rec.get("contact_name")
            or email
            or username
        )
        role = _safe_clean(user_rec.get("role") or user_rec.get("account_role") or "Campaign User")
        field_username = _safe_clean(user_rec.get("field_username") or user_rec.get("username") or username or email)
        seed = email or field_username or username or name
        return {
            "person_id": _safe_clean(user_rec.get("person_id")) or _person_id_from_seed(seed, "auth"),
            "name": name or field_username or email or username,
            "role": role,
            "status": "Active",
            "email": email,
            "field_username": field_username or email or username,
            "username": username or field_username or email,
    ...
    for p in people:
        if not isinstance(p, dict):
            continue
        status_l = _safe_clean(p.get("status") or "Active").lower()
        if status_l in {"active", "prospect", ""}:
            active.append(dict(p))

    # 2) Campaign-scoped app/security users
    try:
        sec = load_security_store() or {}
    except Exception:
        sec = {}
    aliases = _campaign_aliases(sec)
    users_obj = (sec.get("users") or {}) if isinstance(sec, dict) else {}
    if isinstance(users_obj, dict):
        user_items = list(users_obj.items())
    elif isinstance(users_obj, list):
        user_items = [(_safe_clean(u.get("username") or u.get("email")), u) for u in users_obj if isinstance(u, dict)]
    else:
        user_items = []
    ...
    for username, u in user_items:
        if not isinstance(u, dict):
            continue
        role_l = _safe_clean(u.get("role") or u.get("account_role")).lower()
    ...
    for person in active:
        pid = _safe_clean(person.get("person_id"))
        if not pid:
            seed = _safe_clean(person.get("email") or person.get("field_username") or person.get("username") or person.get("name"))
            pid = _person_id_from_seed(seed, "tm")
            person["person_id"] = pid
        if not pid:
            continue
        name = _safe_clean(person.get("name")) or _safe_clean(person.get("email")) or _safe_clean(person.get("field_username")) or "Unnamed"
        role = _safe_clean(person.get("role")) or "Team"
        label = f"{name} — {role}"
        if label in seen_labels:
            extra = _safe_clean(person.get("field_username") or person.get("email") or person.get("username"))
            if extra:
                label = f"{label} ({extra})"
        if label in seen_labels:
            continue
...
def render_program_door_to_door_a3(campaign_id: str, program: dict, people_lookup: dict | None = None, user_id_to_label: dict | None = None, key_suffix: str = "") -> None:
    ...
    with assign_tab:
        ...
        if not saved:
            ...
            st.caption("C4.6.45: Assignment edits are lightweight. The full household/voter JSON is built only when you click Generate, so changing the Assign To picker does not freeze the page.")
            m1, m2 = st.columns(2)
            mobile_ready_key = f"a34_mobile_pkg_ready_{campaign_id}_{pid}_{selected_id}_{key_suffix}"
            with m1:
                # C4.6.45: Do NOT build the heavy mobile package during normal page reruns.
                # Whole-universe packages can contain thousands of households/voters, so the
                # build/export step must happen only inside this button click.
                if st.button("Generate Mobile Assignment Package", type="primary", key=f"a34_generate_mobile_assignment_{campaign_id}_{pid}_{selected_id}_{key_suffix}"):
                    with st.spinner("Building and publishing the mobile assignment package. Large whole-universe packages can take a moment..."):
                    ...
                    if ok:
                        st.session_state[mobile_ready_key] = mobile_pkg
                        st.success("Mobile assignment package uploaded to R2 and available to the Field App now.")
                        st.code(msg)
                    else:
                        st.error(msg)
            with m2:
                ready_pkg = st.session_state.get(mobile_ready_key)
                if ready_pkg:
                    st.download_button(
                    ...
                    st.button("Download Mobile Assignment JSON", disabled=True, key=f"a34_download_mobile_assignment_disabled_{campaign_id}_{pid}_{selected_id}_{key_suffix}")
...
def _gr_is_blank_geo_value(value: str) -> bool:
    """Treat placeholder/reporting fallback labels as blank for real geo matching."""
    val = _gr_clean(value).strip().lower()
    return val in {"", "unknown", "whole universe", "area / best available", "unmatched contact records", "none", "nan"}


def _gr_looks_like_real_voter_id(value: str) -> bool:
    """Return True only for real voter-id-looking values, not voter names.

    Some early mobile rows accidentally saved the voter name in voter_id
    (for example, "Richard A Dellinger" or "James L Shue; Josephine Shue").
    Treating those names as IDs caused Reporting to drop otherwise valid
    mobile contacts from the program/universe join.
    """
    val = _gr_clean(value)
    if not val:
        return False
    # PA voter ids in our data are numeric/hyphen style, e.g. 011937881-67.
    # Be permissive for future IDs, but require real digit content and reject
    # obvious names/household name lists.
    if ";" in val or "," in val:
        return False
    if re.search(r"[A-Za-z]", val) and " " in val:
        return False
    digits = re.sub(r"\D", "", val)
    return len(digits) >= 5


def _gr_household_match_key(row: dict) -> str:
    """Fallback match key based on household geography/address from mobile package."""
    if not isinstance(row, dict):
        return ""
    hk = _gr_clean(row.get("household_key") or row.get("Household Key"))
    if hk:
        parts = [p.strip().upper() for p in hk.split("|") if p.strip()]
        if len(parts) >= 4:
            return "HH:" + "|".join(parts[:4])
        return "HH:" + re.sub(r"\s+", " ", hk.upper())
    county = _gr_clean(row.get("County") or row.get("county")).upper()
    muni = _gr_clean(row.get("Municipality") or row.get("municipality")).upper()
    precinct = _gr_clean(row.get("Precinct") or row.get("precinct")).upper()
    addr = _gr_clean(row.get("household_address") or row.get("res_address") or row.get("Residential Address") or row.get("Address") or row.get("address")).upper()
    if county or muni or precinct or addr:
        return "HH:" + "|".join([county, muni, precinct, re.sub(r"\s+", " ", addr)])
    return ""
...
def _gr_contact_match_key(row: dict) -> str:
    ...
    Important: only real voter IDs are allowed to become VID keys. Older mobile
    rows sometimes wrote voter names into voter_id; those must fall through to
    name/address or household matching so reporting keeps and geo-attributes
    every synced field result.
    """
    if not isinstance(row, dict):
        return ""
    voter_aliases = [
    ...
    for key in voter_aliases:
        val = _gr_clean(row.get(key))
        if val and _gr_looks_like_real_voter_id(val):
    ...
    return _gr_household_match_key(row)
...
def _gr_filter_contacts_to_universe(rows: list[dict], universe_rows: list[dict]) -> list[dict]:
    """Keep report scope tied to the current program/list universe without losing valid mobile contacts.

    A synced mobile row may have a bad voter_id from an older mobile build, but
    still carry an authoritative household_key:
        COUNTY|MUNICIPALITY|PRECINCT|ADDRESS
    That row belongs in the campaign/program universe and must be counted.
    """
    if not rows or not universe_rows:
        return rows or []

    universe_keys = { _gr_contact_match_key(v) for v in universe_rows or [] if _gr_contact_match_key(v) }
    universe_hh_keys = { _gr_household_match_key(v) for v in universe_rows or [] if _gr_household_match_key(v) }
    universe_counties = { _gr_geo_key(_gr_area_value_by_break(v, "County")) for v in universe_rows or [] if _gr_area_value_by_break(v, "County") }
    universe_munis = { _gr_geo_key(_gr_area_value_by_break(v, "Municipality")) for v in universe_rows or [] if _gr_area_value_by_break(v, "Municipality") }
    universe_precincts = { _gr_geo_key(_gr_area_value_by_break(v, "Precinct")) for v in universe_rows or [] if _gr_area_value_by_break(v, "Precinct") }

    if not universe_keys and not universe_munis and not universe_precincts:
        return rows or []

    filtered = []
    for r in rows or []:
        key = _gr_contact_match_key(r)
        hh_key = _gr_household_match_key(r)
        ...
        if hh_key and hh_key in universe_hh_keys:
            filtered.append(r); continue
        # If the household_key says the contact is inside this campaign universe,
        # keep it even when the older voter_id field is malformed.
        if hk_precinct and hk_precinct in universe_precincts:
            filtered.append(r); continue
        if hk_muni and hk_muni in universe_munis and (not universe_counties or not hk_county or hk_county in universe_counties):
            filtered.append(r); continue
        # Keep rows with no match key at all rather than hiding synced results.
        if not key and not hh_key:
            filtered.append(r)
    return filtered
...
def _gr_area_rows_by_break_with_universe(rows: list[dict], universe_rows: list[dict], break_by: str) -> list[dict]:
    ...
    area_label_by_key = {_gr_geo_key(k): k for k in areas.keys()}
    valid_real_areas = [k for k in areas.keys() if _gr_geo_key(k) and not _gr_is_blank_geo_value(k)]
    ...
    for r in rows or []:
        ...
        # For mobile results, household_key is authoritative for County/Municipality/Precinct:
        # COUNTY|MUNICIPALITY|PRECINCT|ADDRESS. Prefer it whenever it is present,
        # because some reporting enrichment passes can leave placeholder or stale
        # assignment/list names on the row.
        if break_by in {"County", "Municipality", "Precinct"}:
            hk_area = _gr_household_key_geo(r, break_by)
            if hk_area and not _gr_is_blank_geo_value(hk_area):
                area = hk_area

        area_key = _gr_geo_key(area)
        if area_key in area_label_by_key:
            area = area_label_by_key[area_key]
        elif not area or _gr_is_blank_geo_value(area) or (valid_area_keys and area_key not in valid_area_keys):
...
with st.sidebar:
    ...
    #   Admin -> existing My Account / Account Admin / Campaign Organization
    #   Voter Lookup -> existing Voter Lookup
    #   Create Universe -> existing Create Universe
    #   Mail Ballot -> existing Mail Ballot Center
    #   Grassroots -> existing Grassroots Center
    #   GOTV -> placeholder routed to existing Grassroots engine for now
    #   Election Day -> existing Election Day placeholder/workspace
    #   Reporting -> existing Area Intelligence for now
    _current_section = st.session_state.get("left_section")

    def _cc_v2_nav_button(label: str, section_value, view_value: str = "dashboard", key: str | None = None, allowed: bool = True):
        if not allowed:
            return
    ...
    with st.expander("📣 Grassroots", expanded=_current_section in {"voter_outreach"}):
        _cc_v2_nav_button("📣 Programs / Assignments / Workers", "voter_outreach", "outreach", key="v2_nav_grassroots")

    with st.expander("🚪 GOTV", expanded=False):
        st.caption("Phase 1 placeholder. Uses existing Grassroots engine until GOTV is split in a later phase.")
        _cc_v2_nav_button("🚪 Open GOTV Workspace", "voter_outreach", "outreach", key="v2_nav_gotv_placeholder")

    with st.expander("🗳️ Election Day", expanded=_current_section in {"election_day"}):
        _cc_v2_nav_button("🗳️ Election Day Operations", "election_day", "election_day", key="v2_nav_election_day")

    with st.expander("📊 Reporting", expanded=_current_section in {"area_intelligence"}):
        _cc_v2_nav_button("⌂ Area Intelligence", "area_intelligence", "dashboard", key="v2_nav_area_intelligence", allowed=user_can("area_intelligence"))
        st.caption("Other report centers will be added after the navigation shell is stable.")
...
# Candidate Connect v2 Phase 4A — Create Universe workflow wrapper.
# IMPORTANT: This preserves the original filter widgets, count engine, saved-universe storage,
# and output/PDF/export engine. The left sidebar remains the source of truth for filters.


def _cc_v2_fmt_int(value):
    try:
...
def _cc_v2_summary_value(summary, key):
    try:
        return int((summary or {}).get(key, 0) or 0)
    except Exception:
        return 0
...
def _cc_v2_filter_chips(active_dict, special_dict):
    ...
    for k, vals in (active_dict or {}).items():
        ...
        chips.append(f"<span class='cc-v2-chip'><b>{html.escape(str(DISPLAY_LABELS.get(k, k)))}:</b> {html.escape(shown)}</span>")

    special_dict = special_dict or {}
    if "RegistrationMonthsAgo" in special_dict:
        chips.append(f"<span class='cc-v2-chip'><b>Newly Registered:</b> last {html.escape(str(special_dict['RegistrationMonthsAgo'].get('max', '')))} months</span>")
    if "__PhoneReach" in special_dict:
        chips.append(f"<span class='cc-v2-chip'><b>Phone Reach:</b> {html.escape(str(special_dict['__PhoneReach']))}</span>")
    if "__ElectionFilters" in special_dict:
        ef = special_dict.get("__ElectionFilters") or {}
        bits = []
        if ef.get("years"):
            bits.append("Years " + ", ".join(map(str, ef.get("years", []))))
        if ef.get("types"):
            bits.append("Types " + ", ".join(map(str, ef.get("types", []))))
        if ef.get("methods"):
            bits.append("Methods " + ", ".join(map(str, ef.get("methods", []))))
        if bits:
            chips.append(f"<span class='cc-v2-chip'><b>Specific Elections:</b> {html.escape('; '.join(bits))}</span>")
    for sf in ["V4A", "V4G", "V4P", "MB_Prob_Score"]:
        if sf in special_dict:
            rng = special_dict.get(sf) or {}
            chips.append(f"<span class='cc-v2-chip'><b>{html.escape(str(DISPLAY_LABELS.get(sf, sf)))}:</b> {html.escape(str(rng.get('min', '')))}–{html.escape(str(rng.get('max', '')))}</span>")
    return chips
...
st.markdown(
    """
    <style>
      .cc-v2-cu-card {background:#f8f4ea;border:1px solid #d6ccb8;border-radius:14px;padding:18px;margin-bottom:14px;box-shadow:0 6px 16px rgba(7,29,58,.07);}
      .cc-v2-cu-card h3 {margin:0 0 8px 0;color:#071d3a;font-size:22px;font-weight:950;}
      .cc-v2-cu-kicker {color:#b01622;font-weight:950;text-transform:uppercase;font-size:12px;letter-spacing:.03em;margin-bottom:8px;}
      .cc-v2-cu-metric {background:#f8f4ea;border:1px solid #d6ccb8;border-radius:14px;padding:16px;min-height:104px;}
      .cc-v2-cu-metric .label {color:#b01622;font-weight:950;text-transform:uppercase;font-size:12px;letter-spacing:.03em;}
      .cc-v2-cu-metric .value {color:#071d3a;font-weight:950;font-size:30px;margin-top:10px;line-height:1;}
      .cc-v2-cu-metric .sub {color:#5f6b7a;font-size:14px;margin-top:12px;font-weight:700;}
      .cc-v2-chip {display:inline-block;background:#dce6e7;color:#071d3a;border-radius:999px;padding:7px 10px;margin:4px 5px 4px 0;font-size:13px;font-weight:800;}
      .cc-v2-cu-link {color:#b01622;font-weight:950;text-decoration:none;}
      .cc-v2-cu-small {color:#5f6b7a;font-size:14px;font-weight:700;}
    </style>
    """,
    unsafe_allow_html=True,
)
...
with m3:
    _filter_group_count = len(_active or {}) + (1 if _special_active else 0)
    st.markdown(f"<div class='cc-v2-cu-metric'><div class='label'>Filters</div><div class='value'>{_cc_v2_fmt_int(_filter_group_count)}</div><div class='sub'>active filter groups</div></div>", unsafe_allow_html=True)
with m4:
    st.markdown("<div class='cc-v2-cu-metric'><div class='label'>Outputs</div><div class='value'>● Ready</div><div class='sub'>role-based export access</div></div>", unsafe_allow_html=True)

_build_tab, _saved_tab, _outputs_tab, _contract_tab = st.tabs(["1. Build Universe", "2. Saved Universes", "3. Outputs", "4. Data Contract"])

with _build_tab:
    left, right = st.columns([1.35, 1.0], gap="large")
    with left:
        st.markdown("<div class='cc-v2-cu-card'><div class='cc-v2-cu-kicker'>Step 1</div><h3>Select filters in the left panel</h3><div class='cc-v2-cu-small'>The original filter widgets are preserved in the sidebar. Filter choices remain campaign-scoped and use the existing filter engine.</div></div>", unsafe_allow_html=True)
        _chips = _cc_v2_filter_chips(_active, _special_active)
        if _chips:
            st.markdown("".join(_chips), unsafe_allow_html=True)
        else:
            st.info("No filters selected. Choose filters in the left panel.")

        st.markdown("<div class='cc-v2-cu-card'><div class='cc-v2-cu-kicker'>Step 2</div><h3>Apply current universe</h3><div class='cc-v2-cu-small'>This calls the existing count engine and saves the current universe for Mail Ballot, Grassroots, GOTV, Election Day, and Reporting.</div></div>", unsafe_allow_html=True)
        a1, a2 = st.columns(2)
        with a1:
            if st.button("Save / Apply Current Universe", width="stretch", key="phase4a_apply_current_universe"):
                with st.spinner("Updating counts with the existing count engine..."):
                    summary, mode, err = update_counts(_active)
                if err:
                    st.warning("Counts are unavailable for this filter combination.")
                    st.caption(str(err)[:500])
                else:
                    ...
                    save_current_universe(_active, summary, source="Create Universe")
                    st.success(f"Current universe saved: {st.session_state.get('current_universe_label', 'Selected universe')}")
                    st.rerun()
        with a2:
            st.button("Clear Filters", width="stretch", key="phase4a_clear_filters", on_click=clear_filter_state)

        st.markdown("<div class='cc-v2-cu-card'><div class='cc-v2-cu-kicker'>Step 3</div><h3>Name and save for reuse</h3><div class='cc-v2-cu-small'>Saved universes use the existing saved-universe store. The type label helps other modules find the right universe later.</div></div>", unsafe_allow_html=True)
        _save_name = st.text_input("Universe name", key="phase4a_save_universe_name", placeholder="Example: High Propensity Persuasion")
...
with _saved_tab:
    ...
    if not saved:
        ...
        rows = []
        for name, payload in sorted(saved.items()):
            payload = payload or {}
            filters = payload.get("filters") or {}
            special = payload.get("special") or {}
            rows.append({
                "Universe": name,
                "Type": payload.get("type", "Universal"),
                "Filter Groups": len(filters or {}) + (1 if special else 0),
                "Created": payload.get("created_at", ""),
            })
        st.dataframe(pd.DataFrame(rows), hide_index=True, width="stretch")
        choice = st.selectbox("Select saved universe", [""] + sorted(saved.keys()), key="phase4a_saved_universe_choice")
        b1, b2 = st.columns(2)
        with b1:
            if st.button("Load Selected Universe", width="stretch", key="phase4a_load_selected_universe") and choice:
                load_saved_universe_into_widgets(saved.get(choice, {}))
                st.success("Loaded saved universe into the sidebar filters.")
                st.rerun()
        with b2:
...
with _outputs_tab:
    ...
    st.caption("This is the existing output/export/PDF engine. No PDF rewrites were made in Phase 4A.")
    _ = render_output_buttons(_active)
...
with _contract_tab:
    ...
    st.markdown(
        """
- Candidate Connect uses the exact app-ready fields produced by the pipeline or mapped during user upload.
- This screen does not guess field-name variants and does not repair duplicate/misnamed fields.
- TSS voter data, MIB daily data, and mapped self-provided data must conform to the CC standard schema before use.
- MIB remains an overlay matched by voter ID; it must not overwrite or pollute the campaign base voter file.
- Missing self-provided fields should disable dependent features with warnings rather than forcing app-side structure fixes.
        """
    )
...
# C4.7.9 Grassroots reporting geo attribution fix: ignores placeholder geo and maps mobile household_key to campaign universe areas.


# C4.7.10 Grassroots reporting: real voter-id validation + household_key geo attribution fix.
